...
sim_path = "simulations_local/exp1"
...
for (i in file.names) {
  ...
  j = str_replace(j, "perceptual,intervention,dynamics", "hsm")
...
for (i in 1:length(file.names)) {
  ...
  file.name = str_replace(file.name, "perceptual,intervention,dynamics", "hsm")
...
save(df.exp1.model, file = "cache/exp1_model_local.RData")
...
sim_path = "simulations_local/exp2"
file.names = dir(sim_path)
...
save(df.exp2.model, file = "cache/exp2_model_local.RData")

# exp 3

df.exp3.model = read.csv(file = "data/model_exp7.csv") %>%
  rename(trial = stimulus) %>%
  filter(trial != 43) %>%
  mutate(index = b)

sim_path = "simulations_local/exp3"
file.names = dir(sim_path)
...
save(df.exp3.model, file = "cache/exp3_model_local.RData")
...
df.exp3.participants = df.exp3.data %>% 
  # filter(codeversion == "experiment_7") %>% 
  pull(datastring) %>% 
