...
class BrickWorld {
    ...
    getBrickPositions() {
        var positions = []
        for (var i = 0; i < this.bricks_array.length; i++) {
            var brick = this.bricks_array[i]
            positions.push([brick.GetPosition().x, brick.GetPosition().y])
        }
        return positions;
    }

    // set asleep based on timer
    setAsleepTimer(timer) {
        var positions = this.getBrickPositions()
        if (timer == 0) {
            this.storedPositions = positions
        } else {
            for (var i = 0; i < this.bricks_array.length; i++) {
                var x = this.storedPositions[i][0] - positions[i][0]
                var y = this.storedPositions[i][1] - positions[i][1]
                var dist = new b2Vec2(x, y)
                var dist = dist.Length()
                // console.log(i, dist)
                if (dist < .1 && this.bricks_array[i].IsAwake()) {
                    // console.log(i, dist)
                    this.bricks_array[i].SetAwake(false)
                }
            }
            this.storedPositions = positions
        }
    ...
    sleepStationaryBricks() {
        ...
        for (let brick of this.bricks_array_set) {
            if (brick.IsAwake()) {
                ...
                var angvel = Math.abs(brick.GetAngularVelocity())
                if (vel < .005 && angvel < .005) {
    ...
    removeBrick(type, level, id) {
        ...
        // using extended noise for everything; perceptual uses goodBricks anyway
        ...
        // if perceptual noise, put bricks to sleep
        var perceptualIdx = types.indexOf('perceptual')
        var do_sleep = false
        if (perceptualIdx != -1 && parseFloat(levels[perceptualIdx]) > 0) {
            do_sleep = true
        }
    ...
    applyNoise(type, level, theBrick, contactList) {
        ...
        var gcount = goodBricks.length

        if (subtypes.indexOf("perceptual") >= 0 && level != 0) {
            for (var i = 0; i < goodBricks.length; i++) {
                var brick = goodBricks[i];
                if (brick == theBrick) {
                    continue;
                };
        ...
        } else if (subtypes.indexOf("intervention") >= 0 && level != 0) {
            // impulse family
            var y_vertical = Math.abs(rand_gauss())
            contactList.forEach(function(brick) {
                if (subtypes.indexOf('360') >= 0) {
                    var x = rand_gauss()
                    var y = rand_gauss()
                } else {
                    var r_vel = -rand_gamma();
                    var t_vel = Math.PI / 2 * Math.random() + Math.PI / 4;
                    x = r_vel * Math.cos(t_vel)
                    y = r_vel * Math.sin(t_vel)
                }
                brick.ApplyImpulse(new b2Vec2(x,y),brick.GetPosition())
            })
        } else if (subtypes.indexOf("dynamics") >= 0 && level != 0) {
            var ctype = "original"
            if (subtypes.indexOf("mul") >= 0) {
                var ctype = "mul"
            } else if (subtypes.indexOf("add") >= 0) {
                var ctype = "add"
            }
            var nl = new CollisionListener(level, getId(theBrick), ctype);
